...
// Função para extrair variáveis do componente React
function extractReactVariables() {
  try {
    const componentPath = path.join(__dirname, '../components/CurriculumPreview.tsx');
    const componentContent = fs.readFileSync(componentPath, 'utf8');
    
    // Extrair classes CSS usadas no componente
    const classMatches = componentContent.match(/className="([^"]+)"/g) || [];
    const classes = classMatches.map(match => match.replace('className="', '').replace('"', ''));
    
    console.log('✅ Classes extraídas do React:', classes.length);
    return {
      classes: classes,
      content: componentContent
    };
  } catch (error) {
    console.warn('Componente React não encontrado');
    return { classes: [], content: '' };
  }
}

// Mapeamento automático JSX → HTML
const jsxToHtmlMap = {
  'className': 'class',
  'htmlFor': 'for',
  'onClick': 'onclick',
...
// Converter JSX para HTML automaticamente
function convertJSXtoHTML(jsxString) {
  let html = jsxString;
  
  // Converter atributos JSX para HTML
  Object.entries(jsxToHtmlMap).forEach(([jsx, html_attr]) => {
    html = html.replace(new RegExp(jsx, 'g'), html_attr);
  });
...
export function generateSyncedHTML(data) {
  ...
  const reactData = extractReactVariables();
  
  console.log('🔄 SINCRONIZAÇÃO AUTOMÁTICA: Extraindo do React...');
  
  // Template base que será preenchido com dados do React
  const baseTemplate = `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <style>
        /* CSS COMPILADO AUTOMÁTICO DO FRONTEND */
        ${compiledCSS}
        
        /* VARIÁVEIS EXTRAÍDAS DO REACT */
        :root {
        ...
        /* GARANTIAS DE RENDERIZAÇÃO - USANDO VARIÁVEIS */
        * {
        ...
        /* CONTAINER PRINCIPAL - USANDO VARIÁVEIS */
        #curriculo-preview {
        ...
        /* LAYOUT FLEX - USANDO VARIÁVEIS */
        .curriculum-flex {
            display: flex !important;
            height: 100% !important;
        }
        
        /* SIDEBAR - USANDO VARIÁVEIS */
        .w-1\\/3 {
        ...
        /* PADRÕES GEOMÉTRICOS - EXTRAÍDOS DO REACT */
        .bg-yellow-400\\/40 {
...
<body>
    <!-- HTML GERADO AUTOMATICAMENTE DO REACT -->
      ...
      <div class="curriculum-flex flex h-full">
        ...
        <div class="w-1/3 bg-gradient-to-br from-slate-800 via-teal-800 to-cyan-800 text-white p-6 relative overflow-hidden">
          ...
          <div class="text-center mb-6">
            <!-- Avatar - ESTRUTURA EXTRAÍDA DO REACT -->
            <div class="w-24 h-24 bg-gradient-to-br from-yellow-400/30 to-white/10 rounded-full mx-auto mb-3 flex items-center justify-center backdrop-blur-sm border-2 border-white/40 relative overflow-hidden">
              <!-- Padrão geométrico - EXTRAÍDO DO REACT -->
        ...
        <!-- Conteúdo Principal - CLASSES EXTRAÍDAS DO REACT -->
