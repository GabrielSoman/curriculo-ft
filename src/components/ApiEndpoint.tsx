...
const ApiEndpoint: React.FC = () => {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Curriculo_${data.nome.replace(/\s+/g, '_')}.pdf`;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-6">
      <div className="flex items-center space-x-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-gray-900">API - Geração Automática</h2>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <h3 className="text-sm font-medium text-blue-900 mb-2">Endpoint da API:</h3>
          <div className="text-xs text-blue-800 space-y-1">
            <div><strong>URL:</strong> <code className="bg-white px-1 rounded">POST /api/generate-pdf</code></div>
            <div><strong>Content-Type:</strong> <code className="bg-white px-1 rounded">application/json</code></div>
            <div><strong>Response:</strong> PDF binário para download</div>
          </div>
        </div>
      </div>
    </div>
